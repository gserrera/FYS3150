...
public:
	...
	void VelocityVerlet(int dimension, int integration_points, double final_time, int print_number, int relativity);
	...
	void GravitationalForceRelativity(planet &current, planet &other, double &Fx, double &Fy);
	void KineticEnergySystem();
	void PotentialEnergySystem();
	void mercury_perihelium(std::ofstream &output, double time);
};
