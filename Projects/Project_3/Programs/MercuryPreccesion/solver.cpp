...
void solver::VelocityVerlet(int dimension, int integration_points, double final_time, int print_number, int relativity)
{   //  Velocity-Verlet solver for two coupeled ODEs in a given number of dimensions.
	...
	char *filenamephi = new char[1000];
	...
	sprintf(filenamephi, "MercuryPeriheliumVV_%d_%.2f.txt", total_planets, time_step);
	std::ofstream output_file(filename);
	std::ofstream output_energy(filenameE);
	std::ofstream output_phi(filenamephi);
	...
	mercury_perihelium(output_phi, time);
	// Set up clock
			...
			for (int nr2 = 0; nr2 < total_planets; nr2++) {
				if (nr2 != nr1) {
					...
					if (relativity = 0) {
						...
						GravitationalForceRelativity(current, other, Fx, Fy);
					}
				}
			}
			...
			for (int nr3 = 0; nr3 < total_planets; nr3++) {
				if (nr3 != nr1) {
					...
					if (relativity = 0) {
						...
						GravitationalForceRelativity(current, other, Fxnew, Fynew);
					}
				}
			}
...
{   // Function that calculates the pure Newtonian gravitational force between two objects, component by component.
...
void solver::GravitationalForceRelativity(planet &current, planet &other, double &Fx, double &Fy)
{   // Function that calculates the relativistic gravitational force between two objects, component by component.
	...
	// Calculate the forces in each direction
	double L = sqrt(current.mass*other.mass*relative_distance[0]* current.mass*other.mass*relative_distance[0]+ current.mass*other.mass*relative_distance[1]* current.mass*other.mass*relative_distance[1])*sqrt(current.velocity[0] * current.velocity[0] + current.velocity[1] * current.velocity[1]);
	Fx -= this->G*current.mass*other.mass*relative_distance[0] / (r*r*r) * (1 + 3 * L*L/(r*r*63285*63285));
	Fy -= this->G*current.mass*other.mass*relative_distance[1] / (r*r*r) * (1 + 3 * L*L/(r*r*63285*63285));
}
...
void solver::mercury_perihelium(std::ofstream &output, double time)
{   // Function that calculates the relativistic gravitational force between two objects, component by component.
	planet &current = all_planets[0];
	double epsilon = 0.3075000000000000000001;
		if (sqrt(current.position[0] * current.position[0] + current.position[1] * current.position[1]) < epsilon) {
			output << "Time" << "\t" << "phi" << std::endl;
			double phi = atan(current.position[1] / current.position[0]);
			output << time << "\t" << phi << "\t" << std::endl;
		}
}
