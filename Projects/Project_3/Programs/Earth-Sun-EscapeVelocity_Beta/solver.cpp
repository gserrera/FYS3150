...
void solver::VelocityVerlet(int dimension, int integration_points, double final_time, int print_number, double beta)
		...
		for (int nr1 = 0; nr1 < total_planets; nr1++) {
			...
			for (int nr2 = 0; nr2 < total_planets; nr2++) {
				if (nr2 != nr1) {
					...
					GravitationalForce(current, other, Fx, Fy, beta);
				}
			}
			...
			for (int nr3 = 0; nr3 < total_planets; nr3++) {
				if (nr3 != nr1) {
					...
					GravitationalForce(current, other, Fxnew, Fynew, beta);
				}
			}
...
void solver::GravitationalForce(planet &current, planet &other, double &Fx, double &Fy, double beta)
{   // Function that calculates the gravitational force between two objects, component by component.
	...
	Fx -= this->G*current.mass*other.mass*relative_distance[0] / std::pow(r,beta+1.);
	Fy -= this->G*current.mass*other.mass*relative_distance[1] / std::pow(r,beta+1.);
}
...
{
	totalPotential = 0;
	for (int nr = 0; nr < total_planets; nr++) { //Sets it to zero at first
		planet &Current = all_planets[nr];
		Current.potential = 0;
	}
	for (int nr1 = 0; nr1 < total_planets; nr1++) {
