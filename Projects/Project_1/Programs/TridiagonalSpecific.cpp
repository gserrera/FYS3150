...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i <= exponent; i++) {
		...
		double h = 1.0 / (n);
		double hh = h * h;
		// Set up arrays for the tridiagonal specific case of Project 1
		...
		finish = clock(); // Finishing time for the algorithm
		double time = ((finish - start) / CLOCKS_PER_SEC); // Conversion of time into seconds
